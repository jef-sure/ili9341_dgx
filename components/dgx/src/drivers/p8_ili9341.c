...
void dgx_p8_ili9341_destroy_screen(dgx_screen_t *scr) {
...
esp_err_t dgx_p8_ili9341_init(dgx_screen_t *scr, gpio_num_t rst, uint8_t color_bits, dgx_color_order_t cbo) {
    ...
    scr->destroy = dgx_p8_ili9341_destroy_screen;
    scr->frame_buffer = 0;
...
//Send all the commands
    ...
    for (uint8_t cidx = 0; lcd_init_cmds[cidx].databytes != 0xff; ++cidx) {
        ...
        dgx_p8_sync_send_data(scr, data, lcd_init_cmds[cidx].databytes);
...
void dgx_p8_ili9341_orientation(dgx_screen_t *scr, dgx_orientation_t dir_x, dgx_orientation_t dir_y, bool swap_xy) {
    ...
    scr->swap_xy = swap_xy;
    dgx_p8_sync_send_cmd(scr, cmd);
    dgx_p8_sync_send_data(scr, data, 1);
}
